...
(8)  Get packages via pip: pip install spyder ipywidgets openpyxl tabpfn lightgbm scikit-learn shap shapiq matplotlib seaborn
(9)  Optional if nvidia via pip: pip install torch --index-url https://download.pytorch.org/whl/cu130 --upgrade
...
Add 'and self.data is not None:' in line 483 after 'if self.feature_perturbation == "tree_path_dependent"'
(2)  Case n_classes and interactions: Only one set of interaction is provided, but not a seperate per class
Open miniconda3\envs\iml\Lib\site-packages\shap\explainers\_tree.py
Change 'if model.n_classes_ > 2:' to 'if model.n_classes_ >= 2:' in line 1415